...
for temp in "${TEMPS[@]}"; do
  ...
  if [ -f /gws/nopw/j04/aopp/andreww/side_projects/ECS_bump/Data/PyRADS/co2_${temp}K.npy ] && [ ! -f /gws/nopw/j04/aopp/andreww/side_projects/ECS_bump/Data/PyRADS/co2_${temp}K_p01_imbalance.npy ]; then
    ...
    sed -i "3 s+.*+#SBATCH --job-name=${temp}K_run +g" ./log/submit_pyrads_${temp}K_p01_imbalance.sbatch
    sed -i "4 s+.*+#SBATCH -o ./log/run_${temp}K.out +g" ./log/submit_pyrads_${temp}K_p01_imbalance.sbatch
    sed -i "5 s+.*+#SBATCH -e ./log/run_${temp}K.err +g" ./log/submit_pyrads_${temp}K_p01_imbalance.sbatch
